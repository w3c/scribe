...
xssVulnerabilityTest5: xssVulnerabilityTest5.txt
	@if ($(PERL) scribe.perl xssVulnerabilityTest5.txt -embedDiagnostics -template testTemplate.html 2> /dev/null | $(GREP) -q -c '<img'); then $(ECHO) 'FAILURE: xss vulnerability test 5 failed'; else $(ECHO) 'xss vulnerability test 5 passed'; fi
